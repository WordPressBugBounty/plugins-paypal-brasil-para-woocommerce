...
//				'validation' => array( $this, 'required_nonce' ),
	...
	public function handle() {
		try {
			...
			WC_PAYPAL_LOGGER::log("Payload create order on SPB from mini cart", $gateway->id, "info", $data);
			
			try {
				$create_payment = $gateway->api->create_payment( $data, array(), 'shortcut' );
				WC_PAYPAL_LOGGER::log("Create order on SPB from mini cart", $gateway->id, "info", $create_payment);
				...
				WC_PAYPAL_LOGGER::log("Error to create order on SPB from mini cart.", $gateway->id, "error", $error_data);
